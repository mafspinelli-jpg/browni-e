...
<div class="escopo">
    ...
    <form method="POST">
        ...
        <button name="cadastrar">Confirmar Pedido</button>
    </form>

    <hr>
    
        <h3>Histórico de Pedidos</h3>
        <input class="input_filto" type="text" id="buscarPed" placeholder="Digite o numero do pedido...">

    <table id="tabelaPed" border="1" cellpadding="5">
        <tr>
            <th>Nº</th>
            <th>Cliente</th>
            <th>Doce / Produto</th>
        ...
        foreach ($historico as $ped) {
            echo "<tr>";
            echo "<td class='num-ped'>" . $ped['id'] . "</td>";
            ...
            echo "<td>
                    <a href='editar_pedido.php?id=" . $ped['id'] . "' style='color: blue;'><img src='/brownie/IMG/pencil.svg' style='width: 20px; height: 20px; vertical-align: middle;'></a> | 
                    <a href='pedidos.php?excluir_id=" . $ped['id'] . "' onclick=\"return confirm('Deseja cancelar esse pedido e estornar o estoque?')\" style='color: red;'> <img src='/brownie/IMG/lixeira.svg'  style='width: 20px; height: 20px; vertical-align: middle;'></a>
                  </td>";
            echo "</tr>";
        }
        ?>
...
<script>
document.getElementById('buscarPed').addEventListener('keyup', function() {
    var busca = this.value.toLowerCase();
    var tabela = document.getElementById('tabelaPed');
    if (!tabela) return;
    
    var linhas = tabela.getElementsByTagName('tr');
    for (var i = 1; i < linhas.length; i++) {
        var celulaNum = linhas[i].querySelector('.num-ped');
        if (celulaNum) {
            var numPed = celulaNum.textContent.toLowerCase();
            if (numPed.indexOf(busca) > -1) {
