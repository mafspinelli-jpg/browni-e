...
<body>
 <div class="menu_lateral">
    ...
    <form method="POST">
        ...
        Nome do Doce: 
        <br>
        <input type="text" name="nome" placeholder="" required> <br>
        Descrição: <br>
        <textarea name="descricao" placeholder="" rows="3"></textarea> <br>
        Preço de Venda (R$): <br>
    ...
    <input  class="input_filto" type="text" id="buscarDoce" placeholder="Digite o nome do produto...">
    ...
    if (!empty($listaDeProdutos)) {
        ...
        foreach ($listaDeProdutos as $p) {
            echo "<tr>";
            echo "<td>" . $p->getId() . "</td>";
            echo "<td class='nome-doce'>" . $p->getNome() . "</td>";
            ...
            echo "<td>
        <a href='editar_produto.php?id=" . $p->getId() . "' title='Editar'>
            <img src='/brownie/IMG/pencil.svg' style='width: 20px; height: 20px; vertical-align: middle;'>
        </a> 
        |
        <a href='produtos.php?excluir_id=" . $p->getId() . "' onclick=\"return confirm('Deseja mesmo excluir este doce?')\" title='Excluir'>
            <img src='/brownie/IMG/lixeira.svg'  style='width: 20px; height: 20px; vertical-align: middle;'>
        </a>
      </td>";
        }
        echo "</table>";
...
<script>
document.getElementById('buscarDoce').addEventListener('keyup', function() {
    ...
    for (var i = 1; i < linhas.length; i++) {
        var celulaNome = linhas[i].querySelector('.nome-doce');
        if (celulaNome) {
            var nomeDoce = celulaNome.textContent.toLowerCase();
            if (nomeDoce.indexOf(busca) > -1) {
