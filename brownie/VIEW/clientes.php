...
// Variável para armazenar as mensagens de feedback na tela
$mensagemFeedback = "";
...
if (isset($_GET['excluir_id'])) {
    ...
    if ($resultadoDel) {
        ...
        $mensagemFeedback = "<div class='alerta alerta-erro'><b>Erro:</b> Falha ao excluir o cliente.</div>";
    }
}
...
if (isset($_POST['cadastrar'])) {
    ...
    // $cpfInformado = $_POST['cpf'];
     $cpfInformado = preg_replace('/[^0-9]/', '',  $_POST['cpf']);

    if ($cliente->validarCpf($cpfInformado)) {
        
        $cliente->setNome($_POST['nome']);
        $cliente->setCpf($cpfInformado);
        $cliente->setEmail($_POST['email']);
        $cliente->setTelefone($_POST['telefone']);
        $cliente->setCidade($_POST['cidade']);
        $cliente->setEstado($_POST['estado']);

        // Usamos o try-catch para capturar erros do banco de dados (PDOException)
        try {
            $resultado = $clienteDAL->Insert($cliente);

            if ($resultado) {
                $mensagemFeedback = "<div class='alerta alerta-sucesso'><b>Sucesso:</b> Cliente cadastrado com sucesso!</div>";
            } else {
                $mensagemFeedback = "<div class='alerta alerta-erro'><b>Erro:</b> Falha ao cadastrar o cliente.</div>";
            }
        } catch (\PDOException $e) {
            if ($e->getCode() == 23000 || strpos($e->getMessage(), '1062') !== false) {
                $mensagemFeedback = "<div class='alerta alerta-erro'><b>Erro:</b> Este CPF já está cadastrado no sistema!</div>";
            } else {
                $mensagemFeedback = "<div class='alerta alerta-erro'><b>Erro no Banco:</b> " . $e->getMessage() . "</div>";
            }
        }

    } else {
        $mensagemFeedback = "<div class='alerta alerta-erro'><b>Erro:</b> O CPF é inválido!</div>";
    }
}
...
<div class="escopo">
    ...
    <input type="text" id="buscarCliente" placeholder="Digite o nome do cliente..." class="input_filto">
  
    <?php
    $listaDeClientes = $clienteDAL->Select();

    if (!empty($listaDeClientes)) {
        echo "<table id='tabelaClientes'>";
        echo "<tr>
                <th>ID</th>
                <th>Nome</th>
                <th>Cpf</th>
        ...
        foreach ($listaDeClientes as $c) {
            echo "<tr>";
            echo "<td>" . $c->getId() . "</td>";
            echo "<td class='nome-cliente'>" . $c->getNome() . "</td>";
            echo "<td>" . $c->getCpf() . "</td>";
            ...
            echo "<td>
                    <a href='editar_cliente.php?id=" . $c->getId() . "' style='color: blue;'><img src='/brownie/IMG/pencil.svg' style='width: 20px; height: 20px; vertical-align: middle;'></a> | 
                    <a href='clientes.php?excluir_id=" . $c->getId() . "' onclick=\"return confirm('Tem certeza que deseja excluir?')\" style='color: red;'><img src='/brownie/IMG/lixeira.svg' style='width: 20px; height: 20px; vertical-align: middle;'></a>
                  </td>";
            echo "</tr>";
        }
        echo "</table>";
...
document.getElementById('buscarCliente').addEventListener('keyup', function() {
    ...
    for (var i = 1; i < linhas.length; i++) {
        var celulaNome = linhas[i].querySelector('.nome-cliente');
        if (celulaNome) {
            var nomeCliente = celulaNome.textContent.toLowerCase();
            
            if (nomeCliente.indexOf(busca) > -1) {
